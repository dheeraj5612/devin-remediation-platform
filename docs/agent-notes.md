...
- Focused registry tests, the full Python suite, Ruff, compilation, and whitespace checks passed before the live run. The candidate result below supersedes the earlier baseline-only state.

## Live issue 9 run, 23 September 2026

- Terminated and archived the three suspended project-tagged Devin sessions, and archived the one project-tagged session that had already exited. Left the unrelated untagged organization session untouched. Devin retains terminated records for reference.
- Restored the fork's existing GitHub `issues` webhook through a webhook-only tunnel. A partial hook update had reset its content type to `form`; setting `content_type=json` and the configured secret made GitHub's real issue-label delivery return HTTP 202. No unsigned local replay was used to create the job.
- Issue [#9](https://github.com/dheeraj5612/superset/issues/9) created live job `32ca494e299743fc9186bb8a9e4e5574`, one Devin session, and [PR #10](https://github.com/dheeraj5612/superset/pull/10). The PR changed only `tests/unit_tests/utils/test_date_parsing.py`; its head SHA is `f2cf1e45117f8ac1859d1dfcfb4569cd1f9e2f35`.
- First validation stopped with `INFRA_ERROR`, not a repair verdict. macOS `/var/folders` versus `/private/var/folders` shortened pytest's collected node ID. Resolving the worktree path before pytest preserves the exact approved ID. Rebuilt all four baselines under the changed harness; all are `CONFIRMED`.
- The new operator-only `revalidate` path checked the unchanged open PR head, reran the detached validator at the saved SHA, and recorded `VERIFIED`: normal `PASS`, mutant `ASSERTION_FAILED`, candidate SHA equals validated SHA. No second Devin session or correction message was sent. The duplicate GitHub delivery was suppressed with one job retained.
- The dashboard is at `http://127.0.0.1:8010/dashboard`; the job proof is `/jobs/32ca494e299743fc9186bb8a9e4e5574`. The worker is stopped after the run. The quick tunnel and local web process are ephemeral; restart them before a later live trigger. Do not treat port `8001` simulation or the historical `/evidence` archive as this job's proof.

## Live batch and leader view, 23 September 2026

- Added issue status write-back (`app/issue_status.py`, `GitHubClient.upsert_issue_comment`): one comment per job, edited in place from `Orchestrator.step()`. Events `ISSUE_STATUS_COMMENTED` / `ISSUE_COMMENT_FAILED`; never changes job status; skipped in simulation. Manual re-sync: `python -m app.cli issue-status --job-id <id>`.
- Added the dashboard leader strip and throughput list (`app/metrics.py`, `app/report.py`). ACUs per verified fix reads an optional `acu_consumed` event detail and otherwise shows "Not reported".
- Live jobs, all `VERIFIED` with zero corrections: #9 -> PR #10 (`32ca494e299743fc9186bb8a9e4e5574`), #2 -> PR #11 (`487bd1ffb48b407a8425289986ef0722`), #5 -> PR #12 (`110dbc3fe2d94cf2bdeafb294ace4db1`), #13 -> PR #14 (`2891e18751e344c0b0ea444990cfc310`).
- New application case `report-anchor-non-string` with trusted oracle `evals/application_cases/report_anchor_type.py`. Manually checked in a temp worktree: baseline `REGRESSION` (list, dict, int crash), TypeError-only partial fix `CONTRACT_FAILED`, upstream fix `c38d4e3` `PASS`, wrong `DRP_CANDIDATE_ROOT` `INFRA_ERROR`. Adding the oracle changed the harness fingerprint; all five baselines were rebuilt and `CONFIRMED`.
- Fork cleanup: closed #1 (duplicate of #5), PRs #3/#4/#6 (superseded), #7 (fixed by merged PR #8). Do not merge batch PRs into `remediation-demo` while other cases are pending: scope checks compare against the pinned baseline.
- Ops: the webhook proxy on 8011 must run from the primary checkout (an earlier copy ran from a deleted worktree and returned 500). `pkill -f "venv/bin/python worker.py"` misses the worker because the process shows the framework interpreter path; kill by PID. Docker was verified under colima; the local docker context was switched from orbstack to colima.
