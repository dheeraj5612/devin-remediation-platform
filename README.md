...
The pinned application oracle evidence is recorded in [`evidence/application-oracle.json`](evidence/application-oracle.json): baseline `dedfe23...` produced the expected `REGRESSION`, and reference `22ec1f...` produced `PASS`, both with trusted provenance. Local source checks used Python 3.14.7, while GitHub Verify ran the control-plane suite under Python 3.12.14. Docker build/start, the full test-quality Superset evaluation, and live provider calls were not executed. Ruff 0.16.8 passed against the checkout. The dependency set was installed into an isolated local environment for these checks; the oracle used a separate prepared Superset checkout and did not call Devin.
